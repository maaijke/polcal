...
def prepare_ms(newmsname: str, new_dir: SkyCoord, type="hba", tmpdir="./"):
    ...
    """
    input_ms = f"data/template_{type.lower()}.MS.tgz"
...
def get_jones(
    ...
    jones = get_station_response(mybeam, times, freqs, station_idx=stidx, rotate=True)
    shutil.rmtree(Path(tmpdir,fname))
    with open(outfile,"w") as newf:
        for iti, ti in enumerate(times.mjd):
            for ifreq, freq in enumerate(freqs):
                myJ = jones[iti, ifreq]
                print(f"{ti} {freq} {myJ[0,0].real} {myJ[0,0].imag} {myJ[0,1].real} {myJ[0,1].imag} {myJ[1,0].real} {myJ[1,0].imag} {myJ[1,1].real} {myJ[1,1].imag}",
                    file=newf,
                )
    return jones
